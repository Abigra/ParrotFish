...
gtest<- seq(0,.3, by=.005)
grazing_df <- data.frame()
#Error in `$<-.data.frame`(`*tmp*`, "gvalue", value = 0) : replacement has 1 row, data has 0 <- fix this after lunch
for(index_g in 1:3){
  g<- gtest[index_g]
  start_values<-f_start_values(g)
  grazing_df$gvalue[index_g] <- gtest[index_g]
  grazing_df$Mfinal[index_g]<- stored_values[1000,1]
  grazing_df$Cfinal[index_g]<- stored_values[1000,2]
}
